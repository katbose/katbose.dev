...
- [x] 20. Clear the gitleaks failure on the pull request
  ...
  - **Resolved without the force-push.** The SARIF artifact from run `33209313424` named all three findings exactly: `generic-api-key` at `apps/web/lib/security/constant-time.test.ts:4`, `apps/web/tests/contact-route.test.ts:94` and `e2e/workers-spike.spec.ts:118`, every one pinned to commit `6f4183cb`, where all three read `0123456789abcdef0123456789abcdef`. That literal is a sequential-hex keyboard walk, not a credential
  - Extended `.gitleaks.toml` with two further **value-pattern** entries — `test-fixture-…-not-a-real-secret` for the current fixtures and `(?:0123456789abcdef){2}` for the retired one. This honours the constraint below: no path-based or commit-based allowlist was added, so a real secret pasted into any of those files is still caught
  - Confirmed green: `secret-scan` passes on both `push` (`33284180710`) and the required `pull_request` event (`33284182160`), where it had failed on every previous run
  - Squashing the branch was the originally proposed method and is no longer required, so no force-push was requested or performed and the branch history is intact. A squash merge into `main` collapses the range regardless, so the retired literal never reaches the default branch
...
- [~] 23. Verify the Phase 1 vendor inventory — **Operator-owned**
  - **Read-only inventory obtained 2026-08-30, no secret values read.** Cloudflare Worker `katbose-web` (`4d6c5b77…`) returns its settings with `compatibility_date` 2026-08-26, `nodejs_compat` + `global_fetch_strictly_public`, observability on at 0.1 head sampling, and thirteen bindings recorded by identifier only: `ASSETS`, `CMS_URL`, `IP_PSEUDONYM_EPOCH`, `IP_PSEUDONYM_KEY`, `NEXT_INC_CACHE_R2_BUCKET` → `katbose-next-inc-cache`, `SENTRY_DSN`, `SLACK_CONTACT_WEBHOOK_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_URL`, `TURNSTILE_SECRET_KEY`, `UPSTASH_REDIS_REST_TOKEN`, `UPSTASH_REDIS_REST_URL`, `WORKER_SELF_REFERENCE`. `SLACK_ALERTS_WEBHOOK_URL` was added later the same day and is confirmed present in the secret list. Zone `katbose.dev` (`c82fdc8c…`) is active with `image_resizing: on`, which is the transform capability the media gate depends on. Supabase `katbose-db` (`ersangtaqrggqldfdbxq`, `ap-south-1`, PostgreSQL 17.6.1.155) is `ACTIVE_HEALTHY` with both expected migrations recorded, RLS on every public table, and only the private `resume` bucket. npm `katbose@0.0.2` resolves to SHA-1 `73f90d862686bdc5792c29440edc3d642eba73ba`, zero dependencies, `bin.katbose`, `engines.node >=20`. PostHog EU project `katbose-posthog` (259247) is ingesting from host `katbose.dev`: 94 `$pageview` and 355 `web_vital` events over the trailing 30 days, with `LCP`, `CLS`, `INP`, `FCP`, `TTFB` and `FID` all present. Sentry project `katbose-web` sits in org `katbose`, region `https://de.sentry.io`
  - Exercise the Upstash limiter through one successful operation and record the documented fail-closed path — **proven** by task 24: a successful limited operation, a `429` when the window was spent, and a fail-closed refusal while Upstash was paused, with Sentry confirming the `degraded` branch rather than the generic catch
  - `#contact-form` delivery is **proven** by task 24 (exactly one message for the labelled probe, plus a second for an unrelated genuine submission after recovery)
  - **Still blocked — de-minified paired Sentry errors.** `find_releases` returns an empty list for the project, so no release has ever been created and no source map has ever been uploaded. Until `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` and `SENTRY_PROJECT` are all present in the Workers Builds environment, de-minified frames are not merely unproven but impossible: the only live issue, `KATBOSE-WEB-2`, shows bundled `worker.js` frames with no release tag. This is the prerequisite, and it is a build-configuration change rather than an error-generation step
  - **Still blocked — `#katbose-alerts` delivery.** Issue alert rule `788648` is enabled and has already fired (last triggered 2026-08-28T02:13:30Z) but carries a single **Email** action and no Slack action, so nothing can reach the channel. Note that `SLACK_ALERTS_WEBHOOK_URL` cannot satisfy this either: no application code reads it, and `apps/web/.env.example` marks it `NOT-YET-WIRED`. The fix is to add a Slack action to that rule in Sentry; once added it self-proves against the already-escalating `KATBOSE-WEB-2` without manufacturing an error
  - Unrelated but found while inventorying: `KATBOSE-WEB-2` is a live production defect, 539+ unhandled `CompileError: WebAssembly.compile()` rejections at `module evaluation(worker)`, still firing every few minutes. It is not a Phase 1 gate and should be tracked separately
...
- [x] 24. Perform the production contact proof — **Operator-owned**, depends on task 23
  - Executed against the production Worker on 2026-08-30 under correlation label `T24-1788048458855`. Turnstile refuses a CDP-driven browser, which is the control behaving correctly, so the tokens came from a human browser session while the observations were verified independently through the Supabase and Sentry APIs
  - Submit one labelled synthetic message with a fresh production Turnstile token; confirm HTTP 200, exactly one Supabase row and exactly one `#contact-form` message — **proven.** Row `428212e1` at `2026-08-30T00:07:40.599Z`, sole row in the table, `SYNTHETIC%` label and probe name confirmed by query; exactly one matching `#contact-form` message at 05:37 IST
  - Replay the exact successful token; confirm HTTP 403 with no extra row and no extra Slack message — **proven.** `403` observed, row count unchanged
  - Exceed the three-per-hour limit from an isolated production test identity; confirm HTTP 429 with no row and no notification — **proven.** `429` observed on a later run whose window was already spent; row count unchanged. Reaching the limiter at all also proves Turnstile accepted a fresh token, since `verifyTurnstileToken` gates the limiter
  - Make Upstash unavailable through a controlled, reversible change; confirm HTTP 503 with no row and no notification — **proven.** Upstash was paused in its console and restarted. A real submission during the pause was refused, no row was written, and no `#contact-form` message arrived. Decisive corroboration: Sentry logged **no** `contact-submit` exception in that window, so the response came from the early `limit.degraded` return rather than the generic catch — exactly the documented fail-closed branch. Recovery is confirmed too: a submission at `10:16:45.936Z`, after the restart, inserted and notified normally
  - Restore every changed production setting and remove or clearly mark the synthetic contact data — **done.** Upstash restored by KatBose; the synthetic row was deleted with a query guarded on both its UUID and the `SYNTHETIC%` label, leaving `rows_total: 1`, `synthetic_rows: 0`, and the unrelated genuine message untouched
  - Identify requests and side effects by non-sensitive correlation labels only; no tokens, webhook URLs, raw IP addresses, email addresses or message content in the retained proof — honoured; only the label, row UUIDs and timestamps are recorded here
  - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6_

- [-] 25. Resolve the registered-zone media gate — **Operator-owned**
  - **Blocker identified precisely 2026-08-30.** `apps/web/app/media/original/[...key]/route.ts` proxies `${SUPABASE_URL}/storage/v1/object/public/media/{key}`, so the gate needs a **public bucket named `media`**. Production has exactly one bucket, the private `resume`, holding zero objects — so neither the original nor the untransformable fixture exists. Zone-side capability is not the problem: `image_resizing` is confirmed `on` for `katbose.dev`
  - Creating the bucket belongs in a migration beside `20260826000200_private_resume_bucket`, but the object bytes cannot be produced from here: uploading requires the Storage API and therefore `SUPABASE_SERVICE_ROLE_KEY`, which is a Worker secret whose value is deliberately unreadable. So this cannot be closed by an agent alone
  - Either create the production `media` fixture plus a deliberately untransformable object and run `scripts/media/media-probe.mjs` to completion, recording the original, transform, cache-hit and forced-fallback results. Two uploads are needed: a valid PNG, and an object with an image extension whose bytes are not a decodable image, so the transform genuinely fails and `onerror=redirect` is exercised rather than assumed
