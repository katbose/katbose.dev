...
- [ ] Lighthouse ≥ 95 on Performance, SEO and Accessibility — **gate committed, awaiting its first passing run.** `pnpm lighthouse` builds the Worker, starts the OpenNext preview, and runs three mobile passes over `/`, `/projects` and `/agent` with `lighthouserc.json` asserting the three category minimums, LCP ≤ 2.5s and bounded CLS on the *worst* run rather than the median. Reports upload as a CI artifact. **First CI run on 2026-08-29 (run `33208259323`) failed and produced real numbers:** Performance 0.82/0.83/0.84 on `/`, 0.92–0.94 on `/projects`, 0.89–0.96 on `/agent`; SEO a consistent 0.92 on all three; LCP 4.3–4.5s on `/` and 2.5–3.3s elsewhere. The consistent SEO figure is a single failing binary audit, most likely `canonical` reporting another hostname because the absolute canonical points at `katbose.dev` while the audit runs against `127.0.0.1` — a localhost artifact rather than a production defect. Performance needs its own measured workstream; the `lighthouse` job is intentionally **not** a required status check until it does. `/contact` is excluded from the blocking gate because it embeds the third-party Turnstile widget and is reviewed manually instead (decision [#100](16-decision-log.md))
