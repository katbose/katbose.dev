...
- [ ] Extend `test:spike:workers` to the full documented Spike A contract ([11-testing-and-ci.md](11-testing-and-ci.md) §11.2.2) — **committed, awaiting its first CI run.** `e2e/workers-spike.spec.ts` now proves ISR serve → stale → background revalidation against the R2 incremental cache, Draft Mode issue/round-trip/revocation plus rejection of a forged `__prerender_bypass`, and `timingSafeEqual` from `node:crypto` including the length-mismatch path. The probes are application routes restricted to a loopback host, and the comparison probe exercises the real `lib/security/constant-time.ts` that Phase 2's preview gate will use (decision [#98](16-decision-log.md)). The OpenNext build does not run on Windows/OneDrive, so Linux CI is the normative proof and this box stays open until that run is green
...
- [ ] Provision/confirm only the accounts needed for Phase 1 and record no credentials in Git ([17-env-vars.md](17-env-vars.md) §17.1.1) — Cloudflare zone/Worker/Images/Turnstile, Supabase, npm and PostHog are confirmed. **Operator-owned and still unconfirmed:** Upstash, Sentry (now including `SENTRY_ORG`, `SENTRY_PROJECT` and `NEXT_PUBLIC_SENTRY_DSN`), `#contact-form` and `#katbose-alerts`. Cloudflare Access, Render and AI Search belong to later phases and must not block this item
...
- [x] Compare repository package contents with the published release (2026-08-27: `katbose@0.0.1` was a 221-byte placeholder that diverged from the workspace. 2026-08-28: `katbose@0.0.2` was published from the corrected workspace package; the downloaded registry tarball exactly matches the 626-byte workspace tarball at SHA-1 `73f90d862686bdc5792c29440edc3d642eba73ba`)
- [x] Manually publish `katbose@0.0.2` and tag the exact merged release commit as `katbose-v0.0.2` so `npx katbose` serves the real card ([19-design-reference.md](19-design-reference.md) §19.4.2 — published 2026-08-28; npm credentials remained local and never entered CI)
...
- [x] Sentry + PostHog repository wiring — browser and server Sentry initialisation, conditional source-map upload with release tagging from the deploy commit, shared idempotent secret redaction across PostHog and both Sentry surfaces, `sanitize_properties`, and the `contact_submitted` event emitted from the contact route without `posthog-node` (decision [#97](16-decision-log.md)). Source-map upload activates only when `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` and `SENTRY_PROJECT` are all present, so an unconfigured build is unchanged
- [ ] Complete vendor verification: PostHog EU ingestion, production `$pageview` host and `web_vital` events are verified. Still unverified and **operator-owned**: one browser and one server error appearing in Sentry with de-minified frames under the same release, Sentry new-issue/spike delivery to `#katbose-alerts`, Slack `#contact-form` delivery, and live Upstash limiter behaviour
...
- [ ] **Contact form protection live: Turnstile + honeypot + rate limit (fail closed) + Slack** — the non-interactive `katbose.dev` widget, public build variable, Worker secret binding and Slack contact binding are provisioned, and the client/server code enforces `action=contact`, canonical-hostname validation and single-use token reset. `apps/web/tests/contact-route.test.ts` now executes the real handler across the whole decision table (26 cases: 400/403/honeypot/429/503, single-insert ordering, notification-failure isolation, trusted-address derivation); it found and fixed a defect where a malformed body returned 503 and reported a server error to Sentry. **Operator-owned and still unverified:** a fresh production token success, replay rejection, live rate limiting, the resulting Supabase row and Slack delivery.
...
- [ ] Design-reference gates pass ([19-design-reference.md](19-design-reference.md) §19.7) — **repository work complete, two operator items open.** The human ⇄ agent crossfade and the smooth disclosure/accordion height animation are implemented; `--dur-crossfade` is no longer an orphaned token and the scroll reveal now derives from its own tokens, both enforced by `tests/motion-tokens.test.ts` (decision [#99](16-decision-log.md)). `e2e/reduced-motion.spec.ts` covers every catalogue interaction, `e2e/keyboard.spec.ts` proves focus-trap cycling in both directions plus a visible focus indicator, and `e2e/layout-stability.spec.ts` asserts CLS = 0 on a fresh session with the intro loader running and the portrait blocked. Canonical `/agent` and `/llms.txt` continue to derive from the one typed manifest. A 2026-08-29 cold-cache run of `katbose@0.0.2` on Windows 10.0.26200 X64 with Node v24.17.0 and npm 11.13.0 exited 0 with the correct card but took 15,507.9 ms, so the under-three-second requirement is measured and still unmet. A machine-assisted comparison against immutable Hackyfolio revision `b37b169f7cdf6686f9c03bfa7b7019e8954686fb` found no copied source in the reviewed implementation scope, but it does not satisfy the required dated, named human review. **Still open:** an under-three-second cold-cache result and human provenance sign-off
- [ ] Spike A registered-zone image transform/cache/original-fallback probe passes — transform and second-request `CF-Cache-Status: HIT` are verified. `scripts/media/media-probe.mjs` now performs all four checks with unauthenticated public GETs and is itself unit tested, but the Supabase original plus forced `onerror=redirect` remain blocked until production has a `media` object and a deliberately untransformable fixture; without the latter the probe reports NOT VERIFIED rather than passing (decision [#101](16-decision-log.md))
...
- [ ] Lighthouse ≥ 95 on Performance, SEO and Accessibility — **gate committed, awaiting its first passing run.** `pnpm lighthouse` builds the Worker, starts the OpenNext preview, and runs three mobile passes over `/`, `/projects` and `/agent` with `lighthouserc.json` asserting the three category minimums, LCP ≤ 2.5s and bounded CLS on the *worst* run rather than the median. **First CI run on 2026-08-29 (run `33208259323`) failed and produced real numbers:** Performance 0.82/0.83/0.84 on `/`, 0.92–0.94 on `/projects`, 0.89–0.96 on `/agent`; SEO a consistent 0.92 on all three; LCP 4.3–4.5s on `/` and 2.5–3.3s elsewhere. **The SEO figure is resolved.** It was one binary weight-1 audit, `robots-txt`, reporting `Unknown directive` for the `Content-Signal` line: Lighthouse validates robots.txt against a safelist that predates the Content Signals Policy. `canonical` scored 1 — the earlier hostname hypothesis recorded here was wrong, and Lighthouse no longer has a cross-hostname canonical rule. That audit is now excluded from the gate while the SEO threshold stays at 95, and robots.txt validity is gated by `apps/web/tests/robots-txt-validity.test.ts` instead (decision [#102](16-decision-log.md)). The reports also never reached CI: `actions/upload-artifact@v4` skips dot-prefixed paths by default, so `.lighthouseci` uploaded nothing on all three failing runs; `include-hidden-files: true` fixes it and an empty upload is now an error (decision [#103](16-decision-log.md)). **Task 22 measured the Performance gap but did not close it.** Under the exact Lighthouse 12.6.1 mobile/simulated profile (412×823, DPR 1.75), changing static browser Sentry to a DSN-gated dynamic runtime improved the unconfigured home median from Performance 0.55 to 0.63, LCP 5.31s to 4.19s, TBT 1.33s to 1.15s, and transfer 569,072 to 499,257 bytes. Against that no-DSN control, production-like configured Sentry still cost 76,959 bytes, 170ms median TBT and 770ms median LCP; the narrow named-export adapter saves 105,270 transferred bytes versus importing the package namespace dynamically. Intro removal did not help, fonts completed well before LCP with `font-display` passing, and the text LCP spent about 90% of simulated time in render delay. Removing first-paint `Reveal` from the hero improved controlled median LCP only 113ms. The final configured three-pass Windows run remained red: `/` Performance 0.51/0.57/0.50 with LCP 5.57/4.71/5.09s; `/projects` 0.54/0.59/0.61 with LCP 4.66/4.39/4.57s; `/agent` 0.60/0.61/0.63 with LCP 4.33/4.33/4.23s. Accessibility and SEO were 1.00 and CLS was at most 0.00017, but Lighthouse warned that this workstation's CPU is slower than expected. Thresholds are unchanged, Phase 1 stays open, and the `lighthouse` job remains intentionally **not** a required status check (decision [#104](16-decision-log.md)). `/contact` remains excluded from the blocking gate because it embeds the third-party Turnstile widget and is reviewed manually instead (decision [#100](16-decision-log.md))
...
- [x] Baseline weekly database + all-bucket Storage backup implementation: PostgreSQL 17 custom archive, exhaustive authenticated S3 copy/check, per-file manifest, zstd + age, R2 readback verification, completion marker and newest-four retention. **Proven end to end on 2026-08-28** by `backup-drill.yml` (run `33190795456`): the real creator and Bash restore ran against local Supabase and a local S3 stand-in, publishing three sets, pruning the oldest, and restoring 5 tables / 6 rows with a 1 KiB database payload and a 1 KiB Storage object both verified by SHA-256. Building that drill found five defects that made a successful backup impossible ([10-backups-and-portability.md](10-backups-and-portability.md) §10.7). Activation against production is tracked by the two gate items below; JSON/MDX export waits for Payload
