...
## 16.28 Phase 1 completion — observability, runtime probes, motion and gates (2026-08-29)

| # | Status | Decision | Reasoning |
| --- | --- | --- | --- |
| 97 | **Current normative** | *Relationship: implements the `contact_submitted` event of #26 without weakening #57's script budget.* **Server-side PostHog events are sent with a plain `fetch` to the documented capture endpoint. `posthog-node` is not installed. Redaction is shared with the browser through one module, and the event carries a pseudonymous `distinct_id` plus non-identifying properties only.** | The Worker already had to drop dynamic Open Graph rendering to stay under the free-plan 3 MiB script limit, so adding an analytics SDK to the server bundle to emit one event is the wrong trade. The capture endpoint is a stable public HTTP API. Two copies of a redaction regex is how one of them silently stops matching, so PostHog and Sentry share a single implementation whose idempotence is tested. |
| 98 | **Current normative** | *Relationship: makes the Spike A contract of #56 permanently verifiable.* **The ISR, Draft Mode and constant-time probes ship inside the application rather than in a throwaway scaffold. The Draft Mode and comparison probes are reachable only over a loopback host, so no production request can reach them; the ISR probe is a `noindex` page outside the typed public route manifest. The comparison probe exercises `lib/security/constant-time.ts`, which Phase 2's preview gate will use in production.** | The original proof came from a scaffold that was deliberately not committed, so nothing detected a regression in behaviour Phase 2 depends on. Draft Mode issues `__prerender_bypass`, which opts the caller out of the ISR cache and would expose unpublished content once a CMS exists, so it cannot be publicly reachable. A loopback check needs no environment variable and therefore cannot be switched on in production by misconfiguration, unlike a feature flag. Probing real shipped code rather than throwaway logic is what makes the pass meaningful. |
| 99 | **Current normative** | *Relationship: completes the interaction catalogue of #47 and protects the gate in #58.* **The human ⇄ agent crossfade is armed on click by `ModeSwitchLink`, never applied on first paint, and only for navigation between `/` and `/agent`. Motion values that cannot be a CSS `var()` are declared in `lib/motion.ts` alongside the token each mirrors, and `tests/motion-tokens.test.ts` fails when a token drifts or loses its last consumer.** | `--dur-crossfade` was declared and unused while the catalogue claimed the crossfade was implemented, and the scroll reveal used numbers that no longer matched its own tokens — both invisible to every existing test. Applying the fade to page content on load would delay Largest Contentful Paint and risk the Lighthouse gate, so the animation is scoped to the one transition it describes. Reading tokens with `getComputedStyle` during render is not an option for a component that also renders on the server, so the mirror is explicit and machine-checked instead of implicit. |
| 100 | **Current normative** | *Relationship: makes the Lighthouse gate of #24 executable.* **`pnpm lighthouse` runs three mobile Lighthouse passes over `/`, `/projects` and `/agent` against the built Worker and fails on the worst run, not the median. `/contact` is excluded from the blocking gate because it embeds the third-party Turnstile widget. Exact `CLS = 0` is asserted by `e2e/layout-stability.spec.ts` in a real browser; the Lighthouse assertion bounds CLS at 0.01 under simulated throttling.** | A stated score target with no mechanism regresses immediately, and a median-based assertion hides a failing run. Gating a first-party performance budget on a third-party script the project does not control would make the pipeline fail for reasons nobody can fix, so that page is reviewed rather than gated. Layout shift is deterministic in a controlled browser and slightly noisy under simulation, so the exact assertion lives where it can be exact. |
| 101 | **Current normative** | *Relationship: makes the remaining image gate of #16 provable rather than assumed.* **`scripts/media/media-probe.mjs` verifies the origin proxy, the transform, the repeated-request cache hit and the forced `onerror=redirect` fallback using unauthenticated public GETs only. The fallback check requires an explicitly supplied untransformable object; without one the probe reports NOT VERIFIED and exits non-zero.** | A transform failure cannot be forced reliably from outside the zone, so the honest options are a real untransformable fixture or an unproven gate — not a check that quietly passes. Keeping the probe credential-free means it can run from any machine and no report can leak a secret. The probe's own logic is unit tested against a stubbed transport, because an untested script that only runs during an incident is how the backup scripts accumulated five fatal defects. |

---

## 16.29 Lighthouse SEO gate — robots.txt validator vocabulary (2026-08-29)

| # | Status | Decision | Reasoning |
| --- | --- | --- | --- |
| 102 | **Current normative** | *Relationship: keeps #100's gate at #24's threshold and preserves #92's ownership of `robots.txt`.* **The Lighthouse `robots-txt` audit is excluded from the blocking gate through `ci.collect.settings.skipAudits` in `lighthouserc.json`. The SEO threshold stays at 95 and no other audit is excluded. robots.txt validity is instead gated by `apps/web/tests/robots-txt-validity.test.ts`, which applies the audit's own line rules to `generateRobotsText()` with `content-signal` added to the directive vocabulary and nothing else relaxed. The same test asserts that the skip list contains only `robots-txt` and that all three category thresholds are still 0.95, so neither the exclusion nor the threshold can widen unnoticed.** | The first CI run of the gate scored SEO exactly 0.92 on `/`, `/projects` and `/agent`, in all three runs of each. The cause was one binary weight-1 audit: `robots-txt`, reporting `Unknown directive` at line 2 for `Content-Signal: search=yes, ai-train=no, use=reference`. Lighthouse 12.6.1 validates robots.txt against a hardcoded safelist that predates the Content Signals Policy, and one weight-1 failure out of the category's total weight of 300/23 is 0.9233 — the observed score, to the digit. The two options considered first were both rejected once the audit was identified. Pointing the gate at production does not change the file, costs the gate its pre-merge and reproducible properties that #100 depends on, and makes it depend on a deployed release. Aligning the audited origin with the canonical fixes nothing, because the `canonical` audit already scored 1: Lighthouse dropped its cross-hostname rule, and the surviving root-URL rule only applies when the canonical and the audited page share an origin. Dropping `Content-Signal` would trade a deliberate rights reservation for three points of a synthetic score; RFC 9309 requires crawlers to ignore unrecognised directives, and Cloudflare documents that validators flag Content Signals with no observed effect on crawling or ranking. Lowering the threshold was refused outright — 0.9233 is exactly what one failing weight-1 audit scores, so a 0.92 threshold would also admit every future single-audit SEO regression. Excluding the one audit whose vocabulary is provably stale, and re-gating the same file in a committed test that runs far more often than the Lighthouse job, is the stronger control rather than the weaker one. |
| 103 | **Current normative** | *Relationship: makes #100's gate satisfy Requirement 9.5.* **The `lighthouse` job uploads `.lighthouseci` with `include-hidden-files: true` and fails the upload when no report is found.** | The `if: always()` upload step ran on all three failing runs and retained nothing: `actions/upload-artifact@v4` skips dot-prefixed paths unless told otherwise, and `.lighthouseci` is dot-prefixed. The reports existed on disk — the job log shows nine of them written — but the only surviving evidence was `No files were found with the provided path`. A gate whose reports cannot be read is not a gate, so an empty upload is now an error rather than a warning. The runner already orders `collect` before `upload` before `assert` precisely so a failed assertion still publishes its reports; that ordering was pointless while the upload discarded them. |

---

## 16.30 Lighthouse Performance workstream — conditional browser Sentry (2026-08-29)

| # | Status | Decision | Reasoning |
| --- | --- | --- | --- |
| 104 | **Current normative** | *Relationship: protects #100's Performance gate and implements the browser half of #26 without enabling performance telemetry.* **Load the Sentry browser runtime only when `NEXT_PUBLIC_SENTRY_DSN` is present, through a dynamic boundary that re-exports only `init`. Keep tracing, profiling, replay and client reports disabled; retain the intro loader, and do not wrap the above-fold hero in `Reveal`. The Lighthouse thresholds and advisory-job status remain unchanged until every run passes.** | A static Sentry import made an unconfigured browser download and execute the SDK. In controlled three-pass home runs, DSN-gating changed median Performance from 0.55 to 0.63, LCP from 5,312ms to 4,194ms, TBT from 1,330ms to 1,151ms, and transfer from 569,072 to 499,257 bytes. On the same final source, enabling the production-like DSN added 76,959 bytes, 170ms median TBT and 770ms median LCP; dynamically importing the package namespace was rejected because its chunk transferred 176,721 bytes, while the named-export adapter transferred 71,451 bytes. Removing the intro was also rejected: its no-intro median LCP was 4,341ms versus 4,194ms with the intro retained. Fonts completed hundreds of milliseconds before LCP, `font-display` scored 1, and LCP load delay/time were both zero; the text LCP instead spent about 90% of simulated time in render delay. Removing `Reveal` from the hero aligns initial paint with the design reference but improved controlled median LCP only 113ms. The final configured Windows run still failed every route's Performance 0.95 and LCP 2.5s assertions, and Lighthouse warned that the workstation CPU was slower than expected. That warning explains why these local absolute scores cannot replace the CI baseline; it does not justify weakening the gate or closing Phase 1. |

---

## 16.31 Lighthouse Performance workstream — compositor-driven intro (2026-08-30)

| # | Status | Decision | Reasoning |
| --- | --- | --- | --- |
| 105 | **Current normative** | *Relationship: refines #104 while retaining its loader and unchanged #100 gate.* **Run the four greetings and overlay exit as one 1000ms CSS opacity timeline. JavaScript performs only the once-per-session mount and post-animation cleanup; the CSS final frame makes the page visible even if main-thread cleanup is delayed. Mirror `--dur-intro` in `INTRO_DURATION_MS`, preserve the reduced-motion skip and `aria-hidden`, and keep the Lighthouse thresholds, URLs, run count and pessimistic aggregation unchanged.** | Linux run `33336413154` selected already-rendered route text as LCP but attributed 79–85% of LCP to render delay: worst values were 3009ms on Home, 2708ms on Projects and 2621ms on Agent. The 1750ms loader used four timer-driven React updates plus a fifth cleanup update, so visible content depended on main-thread scheduling after an opaque overlay. A single compositor timeline removes those intermediate commits and cuts the fixed visibility delay by 750ms while remaining inside #44's two-second interaction budget. The gate remains open until a fresh unchanged-gate run passes all nine reports; the measurements justify the implementation change, not an evidence claim. |

---

## 16.32 Lighthouse Performance workstream — stable optional font paint (2026-08-30)

| # | Status | Decision | Reasoning |
| --- | --- | --- | --- |
| 106 | **Current normative** | *Relationship: narrows #57's delivery behavior and continues #104–#105 without changing #100.* **Self-host DM Sans and JetBrains Mono through `next/font`, but use `font-display: optional` for both. Preserve the declared families, weights, subsets and metric-compatible fallbacks. Fast loads may use the custom faces; constrained loads must keep the fallback rather than swap text late. Keep every Lighthouse threshold, URL, run and aggregation rule unchanged.** | Linux run `33338162574` passed all required checks and Workers Builds. Its Lighthouse traces selected DM Sans text on Home and Projects and JetBrains Mono text on Agent as LCP. Eight of nine observed traces rendered the candidate within 81–128ms, but the simulated mobile model placed final text paint at 2,280–2,760ms; all three routes had at least one LCP above 2,500ms. Font requests completed quickly in the unthrottled trace and `font-display` scored 1, so this is not a missing-font or invisible-text defect. `optional` retains the custom typography when it arrives inside the browser's short block window while preventing a constrained load from replacing an already legible metric-compatible fallback and creating a later LCP candidate. The single Home Performance 0.68 result is separately identified as runner variance: that trace had 2,401ms TBT while its sibling runs had 28ms and 49ms on identical source. |
---

## 16.33 Lighthouse Performance workstream — calibrated intro duration (2026-08-30)

| # | Status | Decision | Reasoning |
| --- | --- | --- | --- |
| 107 | **Current normative** | *Relationship: supersedes only #105's 1000ms duration; its compositor architecture and every other #104–#106 constraint remain current.* **Set `--dur-intro` and `INTRO_DURATION_MS` to 800ms for the complete four-greeting sequence and overlay exit. Preserve all greetings, once-per-session behavior, reduced-motion skip, `aria-hidden`, CSS-owned final visibility, and the unchanged #100 Lighthouse gate.** | On optional-font revision `38faeaa`, Linux run `33474120332` made the fixed delay explicit: Projects produced 2644/2646/2642ms LCP and Agent 2648/2648/2646ms, each about 145ms above the unchanged 2500ms ceiling with little run-to-run variance. The simulated mobile model expands the compositor interval by roughly 2×; reducing the source timeline by 200ms therefore provides about 400ms of headroom while retaining four 200ms greeting windows and staying well inside #44's two-second interaction budget. The first Home trace's 2756ms TBT remains a cold-run JavaScript outlier and is not represented as fixed by this duration change. |