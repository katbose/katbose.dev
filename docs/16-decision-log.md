...
| 60 | Dark palette | **Project-defined and contrast-verified; parity with the reference's dark mode is not claimed** | The measurement captured light mode only. Rather than block the design pass on a second run, the dark mapping is derived from the same Tailwind `gray` ramp and verified against AA — text at 19.27:1 / 13.66:1 / 7.93:1 and focusable boundaries at 4.16:1. A later dark-mode measurement can refine the values, but that is a token edit, not a design change. ([20](20-design-system.md) §20.4.2, §20.18) |
...
| 65 | Theme resolution and the URL | **The resolved theme is never read from a query parameter. `next-themes` (system preference + `localStorage`) is the only source** | Upstream has a `ThemeFromQuery` component that derives theme from the URL. On an ISR-cached route that makes the rendered output vary by query string, so one visitor's parameter can be served to another from a shared cache entry — a correctness bug, not a preference. It also contradicts the toggle behaviour fixed in #46/#51. Query parameters remain legitimate for the human/agent view (`?view=agent`), which is a genuine content variant and is handled as a distinct cache key, not a client-side theme override. |
